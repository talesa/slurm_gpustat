...
def leaderboard(partition: (str, NoneType) = None):
    cmd = "sacct -S2020-11-02-00:00 -E2050-11-02-00:00 --allusers --format user%20,state%10,elapsed%20,AllocGRES%10 --noheader --truncate --allocations"
    if partition:
        cmd += f" --partition={partition}"
    rows = parse_cmd(cmd)
    ...
    for row in rows:
        tokens = row.split()
        if tokens[2] != '00:00:00' and \
                len(tokens) == 4 and \
                tokens[1] in ['CANCELLED+', 'TIMEOUT', 'FAILED', 'RUNNING', 'COMPLETED', 'OUT_OF_ME+']:
            number_of_gpus = int(tokens[3].split('gpu:')[1])
            time_match = p.search(tokens[2])
            usage_per_user[tokens[0]] += \
                number_of_gpus * \
                (float(time_match.group(1) if time_match.group(1) is not None else 0.) * 24. +
                 float(time_match.group(2)) +
                 float(time_match.group(3)) / 60. +
                 float(time_match.group(4)) / 60. / 60.)
